...
# Configuration & Setup
# ------------------------------------------------------------------------------
LOG_FILE="/tmp/ubuntu_setup_$(date +%F_%H-%M-%S).log"
...
echo "========================================================"
echo "      Ubuntu Workstation Deployment & Setup Script       "
echo "========================================================"
...
if [ -f /etc/os-release ]; then
    ...
    log_error "Could not read /etc/os-release. Is this an Ubuntu-based system?"
    exit 1
fi

log_info "Detected Ubuntu ${UBUNTU_RELEASE} (${UBUNTU_CODENAME})"

# List of codenames to try in descending chronological order for fallbacks
CODENAME_FALLBACKS=("resolute" "questing" "plucky" "oracular" "noble" "jammy")
...
# Don't rely on a single `dpkg -s snapd` check — on stock Ubuntu images the
# Snap Store / snapd can be present as a seeded package set that this check
# alone can miss. Look for any of: the snapd package, the snap binary, the
# /snap directory, or leftover snap-store launcher files, and if ANY of
# those are found, run the full purge.
SNAP_PRESENT=0
...
if [ "$SNAP_PRESENT" -eq 1 ]; then
    ...
    # manually installed. Purging snapd can leave some GNOME components
    ...
    # take them out along with snapd if we didn't protect them first.
    log_info "Protecting GNOME desktop packages from autoremove..."
    DESKTOP_GUARD_PKGS=(ubuntu-desktop ubuntu-desktop-minimal gnome-shell gnome-session gdm3 gnome-control-center nautilus gnome-shell-extensions)
    ...
    if echo "${AUTOREMOVE_PREVIEW}" | grep -qiE 'gnome|ubuntu-desktop|gdm3'; then
...
if echo "${FINAL_AUTOREMOVE_PREVIEW}" | grep -qiE 'gnome|ubuntu-desktop|gdm3'; then
